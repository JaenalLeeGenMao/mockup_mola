...
const SportMenu = ({ className = '', playlists, activeIndex = 0, isDark = 0, type = 'vertical', isGray = 0 }) => {
  ...
  return (
    <LazyLoad containerClassName={[styles.menu, className].join(' ')}>
      {playlists.map(({ id }, index) => (
        <div key={id} className={`${styles.menu__dots} ${isDark ? styles.black : styles.white} ${isGray && styles.gray} ${index === activeIndex ? styles.menu__active : ''}`} />
      ))}
