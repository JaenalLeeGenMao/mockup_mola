...
class WatchDesktop extends Component {
  ...
  renderVideo = dataFetched => {
    ...
    try {
      voVolumeInfo = localStorage.getItem('voplayer-volume-info') || '{"muted": false,"volume": 1}'
      if (voVolumeInfo != null) {
        voVolumeInfo = JSON.parse(voVolumeInfo)
      }
    } catch (err) {}
    ...
    if (dataFetched) {
      ...
      if (dataFetched.streamSourceUrl && defaultVidSetting) {
            ...
            <div className={movieDetailContainer} id="videoContainer">
              <div className={videoPlayerWrapper} id="videoWrapper">
                ...
                <div className={playerClass} id="video-player-root">
                  {loadPlayer ? (
                    <div className={videoPlayerContainer}>{this.renderVideo(dataFetched)}</div>
                  ) : (
