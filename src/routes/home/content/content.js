            ...
            <LazyLoad>
              <div className={styles.content__grid_see_more_wrapper} style={isMobile ? moreStyles : null}>
                {isMobile ? (
                  ...
                  <Link to={`/movie-detail/${id}`} className={`${styles.content__grid_see_more_desktop} ${isDark ? styles.black : styles.white} tourMovieDetail`} onClick={type === 'playlists' && this.handleClick}>
                    {type === 'playlists' ? 'discover' : 'view movie'}
                  </Link>
                )}
              </div>
