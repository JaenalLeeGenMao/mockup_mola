      ...
      <div className={s.wrapper}>
        {
          (this.props.articlesDetail.data.video && this.props.videoSetting)
            ...
            :
            (<BackgroundGradient isMobile={true} url={'https://res-sstv01.cheedn.com/image/248f1ef1-f612-4911-83ed-dab67701cb32/image.jpeg'} />)
        }
        <div className={s.main_content}>
          ...
          <div className={s.social_share_section}><SocialShare url={this.props.currentLocation} isMobile={true} /></div>
          <div className={s.caption_section}>
            <div className={s.caption_indicator} ></div>
            <div className={s.caption_text}><q>Highly-rated midfielder Mason Mount has committed his long-term future to Chelsea by signing a five-year contract until 2024 with the club</q></div>
          </div>
          <div className={s.react_markdown_wrapper}><ReactMarkdown source={this.props.articlesDetail.data.content} escapeHtml={true} /></div>
          <div className={s.tag_section}>
            {
              (this.props.articlesDetail.data.tags && this.props.articlesDetail.data.tags.length)
              &&
              (
                <>
                  {
          ...
          <div className={s.section_box_wrapper}>
            {
              (!this.props.related.isLoading && this.props.related.data.length)
                ?
                (
                  <>
                    <div className={s.title_text_box}>Related Articles</div>
                    {
                      this.props.related.data.map((related) => <ArticleCard isMobile={true} data={related} key={related.id} />)
                    }
                  </>
                )
                :
          ...
          <div className={s.section_box_wrapper}>
            {
              (!this.props.latest.isLoading && this.props.latest.data.length)
                ?
                (
                  <>
        ...
        <div className={s.related_video_section}>
          {
            (this.props.relatedVideoData && this.props.relatedVideoData.length)
            &&
